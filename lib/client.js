		...
		function apply(ctx) {
			...
			function SettingsPanel() {
				...
				function speakText() {
					...
					if (!text) return;
					fetch("/api/la-feng/tts", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ text })
					})
					.then(function (r) { if (!r.ok) throw new Error("TTS 服务未启动？"); return r.blob(); })
					.then(function (blob) {
						const url = URL.createObjectURL(blob);
						if (ttsAudioRef.current) { ttsAudioRef.current.pause(); ttsAudioRef.current.src = url; }
						else { ttsAudioRef.current = new Audio(url); }
						ttsAudioRef.current.play().catch(function (e) { console.log("[la-feng] play error:", e); });
					})
					.catch(function (e) {
						console.log("[la-feng] tts error:", e);
						alert("语音服务未启动，请先运行 tts_server.py");
					});
				}
				...
				return e("div", null,
					...
					e("div", { style: s },
						...
						e("div", { style: row },
							e("label", { style: label }, "🔊 语音朗读（IndexTTS-2）"),
							e("input", { type: "text", value: ttsText, style: input, placeholder: "输入想听的话，点朗读", onChange: function (ev) { setTtsText(ev.target.value); } }),
							e("div", { style: { display: "flex", gap: 6, marginTop: 6 } },
								e("button", { style: btn, onClick: speakText }, "🔊 朗读"),
								e("button", { style: btn, onClick: stopSpeak }, "⏹ 停止")
							)
						),
			...
			function ImageFab(props) {
				...
				const [pos, setPos] = react.useState({ x: Math.max(0, window.innerWidth - 150), y: 120 });
				const dragRef = react.useRef(null);
				...
				return e("div", { style: { position: "fixed", left: pos.x, top: pos.y, zIndex: 9999, pointerEvents: "auto", fontFamily: "inherit", cursor: "grab" }, onPointerDown: onPointerDown },
					...
					e("button", { style: { display: "flex", alignItems: "center", gap: 6, padding: "10px 16px", border: "none", borderRadius: 22, background: "#6b8afd", color: "#fff", cursor: "pointer", fontSize: 14, boxShadow: "0 4px 16px rgba(0,0,0,0.25)" }, onClick: function () { if (!movedRef.current) setOpen(!open); } }, "🖼 看图")
				);
			}
