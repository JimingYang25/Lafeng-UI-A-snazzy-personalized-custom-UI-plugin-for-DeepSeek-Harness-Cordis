...
function apply(ctx) {
  ...
  const emojiMarkdown = (name) => "![](" + emojiBaseUrl() + "/api/la-feng/emoji?id=" + encodeURIComponent(emojiId(name)) + ")";
  ...
  if (tools !== undefined) {
    ...
    ctx.effect(() => tools.register({
      name: "diary_write",
      description: "把关于用户的重要事实（身份、能力、偏好、项目、工作流等）追加到持久记忆日记（la-feng-diary）。当对话中了解到用户的新事实时使用，避免遗忘。",
      parameters: {
        type: "object",
        properties: {
          entry: { type: "string", description: "要记录的事实内容，建议带分类前缀，如「[身份] ...」「[项目] ...」「[偏好] ...」" }
        },
        required: ["entry"],
        additionalProperties: false
      },
      output: {
        ...
        render: (_args, value) => [{ type: "text", text: (value && value.ok) ? "已记入持久记忆日记" : "记日记失败" }]
      },
      async execute(args) {
        ...
        try {
          ...
          mkdirSync(join(homedir(), ".dsh"), { recursive: true });
          appendFileSync(DIARY_FILE(), "[" + stamp + "] " + entry + "\n");
          return { ok: true };
        } catch (e) {
          return { ok: false };
        }
      ...
      },
      {
        ...
        handler: (req, res) => {
          try {
            ...
            let name = file;
            if (!name && id) {
              for (const [fn, fid] of Object.entries(EMOJI_ID_MAP)) {
                if (fid === id) { name = fn; break; }
              }
              if (!name) name = id;
            }
          ...
          req.on("end", async () => {
            try {
              ...
              if (!text || typeof text !== "string" || !text.trim()) {
                return json(res, 400, { ok: false, error: "no-text" });
              }
              // 转发到本地 IndexTTS-2 常驻服务（tts_server.py）
              const resp = await fetch("http://127.0.0.1:11435/tts", {
                method: "POST",
                headers: { "content-type": "application/json" },
              ...
              if (!resp.ok) {
                ...
                return json(res, 502, { ok: false, error: "tts: " + String(detail).slice(0, 200) });
              }
